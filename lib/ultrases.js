...
UltraSES.prototype.send = function(data, done) {
	...
	var err = util.checkRequiredParamsSend(data); //Verify we have enough to send an email
	if(err)
		return done(err);
	var params = {
	...
	if(data.replyTo){ // ReplyTo is special, because you can't just give it an empty array
...
UltraSES.prototype._merge = function(data, defaults) { //Ever heard of undescore?
	if(!data){
		data = {};
	}
	if(!defaults){ 
